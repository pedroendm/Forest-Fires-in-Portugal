...
add_new_values <- function(ds){
  for(i in 1:length(ds$lat)) {
    ...
    #ds$prcp[i] = add_prcp(nearby_stations,  ds$alert_date[i])
    if(i == nrow(ds))
      break
  }
  return(ds)
...
4.1) Random Forests
```{r Random Forests}
costs = table(ds$intentional_cause)
costs[1] = 1
costs[2] = 100000
...
preds$predictions = as.factor(as.integer(preds$predictions[1:nrow(ds_test)] < 0.515))
