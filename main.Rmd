...
<!-- TODO: don't remove district -->
Regarding the feature *region*, which is the one with more missing values (1206 in total), we don't need to worry about, since we also have the coordinates of the location of the fire, in the attributes *lat* and *lon*, which completely determine the *region* and, also, the features *district*, *municipality* and *parish*, so we can remove all of them, since they are redundant:
```{r Removing region district municipality and parish}
ds <- select(ds, -c(region, municipality, parish))
```
...
``` {r Remove observations with missing values}
ds <- drop_na(ds)
find_na(ds)
```
As one we can see, we don't have any more missing values.
...
Only 3 observations weren't rightful parsed, which correspond to the IDs 2522, 6817 and 8690. Looking in the original data set, they have the following values for the *lon* attribute, respectively: 0.29930555555555555, 0.36041666666666666 and 0.31319444444444444. Looking at the values, they seem to already in a decimal representation. But, with further analysis as we will show after, we found this values to be outliers. So we may just drop it, already:
```{r Dropping NAs again}
ds <- drop_na(ds)
```
...
getPartOfDay <- function (datetime) {
  classes <- factor(c('morning', 'afternoon', 'evening', 'night'), ordered = TRUE)
...
Finally, we checked if there are outliers. Namely regarding the *lat* and *lon* attributes, as one can see in the boxplots below. We considered a value to be an outlier if it's out of [1st Quartile - 1.5\*IQR, 3st Quartile + 1.5\*IQR]. 
...
WHY WE DONT REMOVE THE OTHERS, so we just removed the outliers regarding the *lat* and *lon* attributes:
```{r Remove outliers from lat and lon attributes}
lat_outliers <- boxplot.stats(ds$lat)$out
lon_outliers <- boxplot.stats(ds$lon)$out
ds = ds[-which(ds$lat %in% c(lat_outliers) | ds$lon %in% c(lon_outliers)),]
```

Relatively to the data cleaning and pre-processing, we think that we are good. 
We're still missing the normalization of the values, but that will be done after a exploratory analysis.
...
```{r}
ggplot(ds, aes(x = village_area)) + geom_histogram(binwidth = 3000) + facet_grid(part_of_day ~ season) + ggtitle("Histogram of village area burned by part of the day and season")
```
...
Before we start with the predictive modelling, we decided to normalize our numeric variables, using the *z-score* method, in order to avoid attributes of larger magnitude dominating any aggregation function used in the predictive models.

```{r Values normalization}
ds <- mutate(ds, lon                   = as.numeric(transform(lon, method = "zscore")),
                 lat                   = as.numeric(transform(lat, method = "zscore")),
                 village_area          = as.numeric(transform(village_area, method = "zscore")),
                 vegetation_area       = as.numeric(transform(vegetation_area, method = "zscore")), 
                 farming_area          = as.numeric(transform(farming_area, method = "zscore")),
                 time_combating        = as.numeric(transform(time_combating, method = "zscore")),
                 time_till_firstInterv = as.numeric(transform(time_till_firstInterv, method = "zscore")))
```
...
```{r}
set.seed(123)
inTrain <- createDataPartition(y = ds$intentional_cause, p = 0.7, list = FALSE)  # stratified random split of the data.
