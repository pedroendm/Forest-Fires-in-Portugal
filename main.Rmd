...
Finally, the attributes related to the extinction and first intervation, since there are only a few missing cases, we didn't bother imputating the data and probably adding bias to our data, so we just removed the observations where the missing values appeared.
...
We'll now gather some data, regarding the temperature and the precipitation of the fires:
```{r Temp and Prcp}
```
...
#m <- bagging(intentional_cause ~ ., ds_train, mfinal = 5, control = rpart.control(maxdepth = 3))
...
4.1) Random Forests
```{r Random Forests}
m <- randomForest(intentional_cause ~ ., ds_train, importance = TRUE)
preds <- predict(m, ds_test)
confusionMatrix(preds, ds_test$intentional_cause)
```
...
4.4) XGBoost
```{r}
m <- xgboost(data = data.matrix(select(ds, -intentional_cause)),
             label = as.integer(ds$intentional_cause)-1,
             #max.depth = 2, eta = 1, nthread = 2,
             nrounds = 100,
             objective = "binary:logistic",
             eval_metric = "logloss")

preds <- predict(m, data.matrix(select(ds, -intentional_cause)), reshape=T)

preds.class <- as.numeric(preds > 0.5)

confusionMatrix(factor(preds.class), factor(as.integer(ds$intentional_cause)-1))
```
